...
if ! command -v dpkg-deb >/dev/null 2>&1; then
    print_error "dpkg-deb tidak ditemukan. Pastikan dpkg terinstall."
    exit 1
fi

print_info "Menyiapkan instalasi $DEB_FILE ke $TARGET_PREFIX..."
mkdir -p "$TEMP_DIR" "$EXTRACT_DIR" "$BACKUP_DIR"

# Ekstrak file .deb menggunakan dpkg-deb
print_info "Mengekstrak file .deb menggunakan dpkg-deb..."
if ! dpkg-deb -x "$DEB_FILE" "$EXTRACT_DIR"; then
    print_error "Gagal mengekstrak file .deb"
    exit 1
fi
